...
; clear all relevant hardware registers & wram
	push hl ; preserve all registers
	push de
	push bc
	push af
	call MusicOff
...
.env_ptrn
	; NOTE: this function is unorthodox on Gameboy (Color) titles
...
; load music
	...
	inc hl
	ld [hl], d ; (always 0)
	ld hl, Music ; 3-byte pointers (bank, address)
	add hl, de
	add hl, de
	add hl, de
	ld a, [hli]
...
; to achieve this, check for non-MAX_VOLUME before muting
	...
	push af
	ld a, 1 << SOUND_PRIORITY_F ; stop playing music
	ld [wSFXPriority], a
	pop af
...
; start reading sfx header for # chs
	...
	inc hl
	ld [hl], d
	ld hl, SFX ; 3-byte pointers (bank, address)
	add hl, de
	add hl, de
	add hl, de
	; get bank
...
; functionally identical to InitSound, farcalled by Function29fc9, in "engine/link/mystery_gift"
	ld hl, rNR50
	xor a
	ld [hli], a
	ld [hli], a
...
;	sound channel   1      2      3      4
	...
	ld [hli], a ; rNR14, rNR24, rNR34, rNR44 ; restart sound (freq hi = 0)
	ret
	; trainer music subroutine merged
